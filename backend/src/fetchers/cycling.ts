...
export async function fetchCyclingEvents(): Promise<SportEvent[]> {
  try {
    ...
    // PCS rider page: find the "Upcoming races" section or season participation list
    // Try multiple strategies in order of reliability

    // Strategy 1: look for upcoming section heading then nearby list items
    $('ul.rdrSeasonList li, .rdrUpcoming li').each((_, el) => {
      ...
      if (!dateText || !raceName || isNumeric(raceName)) return;
    ...
    // Strategy 2: scan all tables, look for rows where col[0]=date, col[2 or 3]=race name link
    if (events.length === 0) {
      ...
      $('table').each((_, table) => {
        $(table).find('tr').each((_, row) => {
          ...
          if (!parsePCSDate(dateText)) return; // skip rows where first cell isn't a date
          // Race name is usually in cell index 2 or 3 (after date, country, then race)
          let raceName = '';
          let raceUrl = '';
          ...
          if (!raceName || isNumeric(raceName)) return;
          const date = parsePCSDate(dateText);
          if (!date || date < now) return;
          const key = `${dateText}_${raceName}`;
          if (seen.has(key)) return;
